...
- **Probe** (`pi/checker.py`): runs every 10 minutes on a Raspberry Pi on a home Globe line. It tests plain DNS, DoT and DoH against every resolver in `pi/config.json`, checks for DNS hijacking, and checks for SNI/IP filtering. Writes `pi/out/latest.json` and `pi/out/history.json`. Needs only python3 and curl.
- **Publish** (`pi/publish.sh`): force-pushes `pi/out/` to the `data` branch as a single parentless commit through Tor.
...
5. **Never commit `pi/out/`.** It is gitignored. It holds a separate git repo that pushes with the Pi's deploy key.
6. **No secrets in the repo.** The push credential is an SSH deploy key that lives only on the Pi (`~/.ssh/phblock`).
